...
import type { FasilitasRecord } from "@models/fasilitas";
...
export default function FasilitasClient({
  ...
  initialData: Record<string, FasilitasRecord[]>;
}) {
  ...
  async function handleDelete(id: number, sekolah: string) {
    try {
      await deleteFacility(id);
      setData((prev) => ({
        ...prev,
        [sekolah]: prev[sekolah].filter((item) => item.id),
      }));
    } catch (err) {
                ...
                data[sekolah.key].map((item) => (
                  ...
                  >
                    ...
                    <button
                      ...
                      className="absolute top-2 right-2 bg-white/90 text-red-600 hover:text-red-800 hover:bg-red-50 p-1.5 rounded-full shadow"
                    >
                      <svg
                        className="w-4 h-4"
                        fill="none"
